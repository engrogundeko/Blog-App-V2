...
from .schema import Error, CategorySchema, PostSchema
...
def create_account(request, username: str = Form(...), password: str = Form(...)):
    try:
...
def upload_image(
    ...
    media = Images.objects.create(
        post=qs, alt_text=alt_text, description=description, link=file
    )
